it('Login and cant change pass', () => {
  ...
  cy.get('.form.confirm-password input').clear().type('admincool11');
  cy.get('.content > button').click();
  cy.url().should('include', 'profile-settings');
  cy.get('.form.confirm-password .error-text').should('exist');
  cy.get('.form.confirm-password .error-text').should('have.text', 'Введенные пароли не совпадают');
});
